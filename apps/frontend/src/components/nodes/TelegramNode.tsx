...
const TelegramNode = memo(({ data, selected }: NodeProps) => {
  ...
  return (
    <div className="relative group">
      ...
      <div
        className={`relative bg-gradient-to-br from-cyan-50 to-cyan-100 w-28 h-24 border-2 transition-all duration-300 flex items-center justify-center ${
          isTrigger ? 'rounded-l-full rounded-r-lg' : 'rounded-lg'
        } ${
          selected
            ? 'border-cyan-500 shadow-[0_0_20px_rgba(6,182,212,0.5)] scale-105'
            : 'border-cyan-300 shadow-[0_4px_12px_rgba(6,182,212,0.2)]'
        } hover:border-cyan-400 hover:shadow-[0_6px_16px_rgba(6,182,212,0.3)] hover:scale-102`}
      >
        {/* Input Handle */}
        {!isTrigger && (
          <Handle
            ...
            className="absolute top-1/2 -translate-y-1/2 -left-2
                       bg-cyan-100 border-2 border-cyan-400 w-3 h-3 rounded-full
                       hover:scale-125 hover:border-cyan-500 transition-all duration-200 hover:bg-cyan-200"
          />
        )}
